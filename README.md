Понял, давай вообще без «кусков», один цельный README.md, который ты просто целиком копируешь и вставляешь. Ниже — полностью готовый вариант.
...
3. For invalid coordinates, calls the **OpenCage Geocoding API** (REST) to map `lat`/`lng` from `city` + `country`.
...
Input path inside the container (directory with multiple subfolders, each containing Parquet files):
...
docker-compose exec spark-master bash -c "/opt/spark/bin/spark-submit \
...
Implementation idea:
	•	Split restaurants into valid (lat IS NOT NULL AND lng IS NOT NULL) and invalid (lat IS NULL OR lng IS NULL).
	•	Log counts and sample invalid rows for README / debugging:
	•	valid restaurants count;
	•	invalid restaurants count;
	•	sample invalid rows.
	•	Demonstrate a call to OpenCage Geocoding API for at least one invalid row (e.g. take city, country, call API, print returned lat, lng).
	•	In the main pipeline we proceed with valid restaurants only (rows with non-null coordinates).
...
Implementation idea:
	•	Use Python library geohash2 and define a small helper:
...
Implementation idea:
	•	Output path inside container, e.g.:
...
High-level orchestration in jobs/main.py:
	...
	8.	Task 4: write enriched data as partitioned Parquet (year/month/day) to /app/data/output/enriched_parquet.
	9.	Stop Spark session.

This structure keeps the job simple, readable, and easy to test.
...
7.1. What is tested
	...
	•	Coordinate processing logic (process_restaurant_coordinates-style function):
	•	correctly splits restaurants into valid / invalid by lat / lng;
	•	uses a mocked version of geocode_with_opencage in tests (no real HTTP calls).
	•	Geohash enrichment (add_geohash_columns-style function):
...
Вот это уже один цельный README — просто выделяешь весь блок от `# Spark Practice…` до последней строки и вставляешь в `README.md`.